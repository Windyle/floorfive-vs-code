...
export class BaseModule {
  ...
  /**
   * Gets the icon representing the module.
   * @returns {string} The module's icon.
   */
  ...
  public getActionsHtml(): string {
    if (
      Object.keys(this.commands).length === 0 ||
      ...
      ${Object.keys(this.commands)
        .map((id: string) => {
          ...
          if (command.show()) {
            ...
            return `<button class="command-button ${
              command.isSubCommand() ? "sub" : ""
            } icon-button" ${customStyle} id="${this.getId()}-${command.getId()}" loader="${command.getWithLoader()}"><icon name="${command.getIcon()}"></icon> <label>${command.getLabel()}</label></button>`;
          }
        })
        .join("\n")}
  </div>
