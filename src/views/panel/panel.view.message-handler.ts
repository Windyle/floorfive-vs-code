...
export class PanelViewMessageHandler {
  ...
  public setActivePanel(webview: vscode.Webview, moduleId: string, commandId: string): string {
    try {
      ...
      if (Modules.getModule(moduleId).commands[commandId] !== undefined) {
        content = Modules.getModule(moduleId).commands[commandId].getLogContent();
      } else {
        for (const command of Modules.getModule(moduleId).getCommandsArray()) {
          if (command.getPanelId() === commandId) {
