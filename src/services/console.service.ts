...
export class FFConsole {
  ...
  private _logTypes: { [method: string]: any } = {
    ...
    info: (message: string): string => {
      return `<pre style="color: var(--vscode-terminal-ansiBlue);">${message}</pre>`;
    },
    step: (message: string): string => {
      return `<pre><span style="color: var(--vscode-terminal-ansiBlue);">${
        message.split(" ")[0]
      }</span> <span style="color: var(--vscode-terminal-ansiBrightBlue);">${message
        .split(" ")
        .slice(1)
        .join(" ")}</span></pre>`;
    },
  ...
  private consoleCommandFormat = (message: string): string => {
    ...
    const mainArgument =
      splitMessage[1] !== undefined
        ? `<span style="color: var(--vscode-terminal-ansiYellow)">${splitMessage[1]}</span>`
        : "";
   ...
   * @param type The type of the message (Pre-configured: plain, consoleCommand, alert, warning, error, success, step, color).
   */
